...
uint256 hashGenesisBlock("0x6d2a82a61e4c21fdeadb58d1885422a28168ff5d6567422400431fc3261f99c0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Knavecoin: starting difficulty is 1 / 2^16
...
const string strMessageMagic = "Knavecoin Signed Message:\n";
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Knavecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Knavecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if		(DiffMode == 1) { return GetNextWorkRequired_OLD(pindexLast, pblock); }
...
unsigned int static GetNextWorkRequired_OLD(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Knavecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Knavecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Knavecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "N.Y. Times: U.S. Economy’s Growth Was Slower in Fourth Quarter";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1393477793;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 428585078;
        ...
        assert(block.hashMerkleRoot == uint256("0xb2746c8b309f3e8fffe8e97bc0e6b092eaf1969c885b74206febe9437d94e82b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x0b, 0xd0, 0xc6, 0xeb }; // Knavecoin: increase each by adding 16 to litecoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KnavecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KnavecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KnavecoinMiner(CWallet *pwallet)
{
    printf("KnavecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("knavecoin-miner");
    ...
    try { loop {
        ...
        printf("Running KnavecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        uint256 hashTarget = CBigNum().SetCompact(pblock->nBits).getuint256();
	unsigned char Nfactor = pblock->GetNFactor();
        loop
        {
    ...
    {
        printf("KnavecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KnavecoinMiner, pwallet));
}
